...
void Application::ProcessKeyReleased(sf::Event a_event)
{
	...
	int randomMass = lowest + int(range*rand() / (RAND_MAX + 1.0));

	if (b_addSphere) {
		uint count = m_pEntityMngr->GetEntityCount();
		m_pEntityMngr->AddEntity("Planets\\01_Mercury.obj", "Mercury_" + std::to_string(count + 1));

		vector3 v3Position = m_v3NewTargetToLookAt;
		v3Position.y = 3.0f;
		matrix4 m4Position = glm::translate(v3Position);
		m_pEntityMngr->SetModelMatrix(m4Position);
		m_pEntityMngr->UsePhysicsSolver();
		m_pEntityMngr->SetMass(randomMass);
		b_addSphere = false;
	}
...
void Application::CameraRotation(float a_fSpeed)
{
	...
	glm::quat m_qRotation = glm::angleAxis(-3.0f * fAngleY, -AXIS_Y);

	// and change X axis
	// set to -AXIS_X so that moving mouse to left actuallt rotates toward the left, etc.
	m_qRotation = glm::angleAxis(3.0f * fAngleX, m_qRotateCamera * -AXIS_X) * m_qRotation;
...
#pragma region Camera Position
	...
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space))
	{
		...
		m_pEntityMngr->AddEntity("Minecraft\\Cube.obj", "Cube_" + std::to_string(entityCount + 1));
		vector3 v3Position = vector3(glm::sphericalRand(45.0f));
		v3Position.y = 1.0f;
		matrix4 m4Position = glm::translate(v3Position);
		m_pEntityMngr->SetModelMatrix(m4Position);
		m_pEntityMngr->UsePhysicsSolver();
