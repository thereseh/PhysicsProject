...
bool Simplex::MyEntity::IsColliding(MyEntity* const other)
{
	...
	if (GetEntityType() == Ground) return false;
...
void Simplex::MyEntity::SetEntityType(uint a_mType)
{
	if (a_mType == 0) {
		m_myType = Blackhole;
		if (m_pSolver) m_pSolver->SetIsBlackHole();
	}
	if (a_mType == 1) m_myType = Block;
	if (a_mType == 2) {
		m_myType = Ground;
		if (m_pSolver) m_pSolver->SetIsGround();
	}
}
...
{
	// get the distance
	// "Seeking" we want targetPos - ownPos
	vector3 v3distanceOther = GetPosition() - a_pOther->GetPosition();
	vector3 v3distance = a_pOther->GetPosition() - GetPosition();

	// if they are of same type (Steves, or Zombies)
	// then just check if they need to separate
	if (m_pSolver->IsBlackHole() && a_pOther->m_pSolver->IsBlackHole()) {
		m_pSolver->BlackHolePull(v3distance, a_pOther->GetMass());
		a_pOther->m_pSolver->BlackHolePull(v3distanceOther, GetMass());
	}
	else {

		if (m_pSolver->IsBlackHole()) {
			a_pOther->m_pSolver->BlackHolePull(v3distanceOther, GetMass());
		}
		else if (a_pOther->m_pSolver->IsBlackHole()) {
			m_pSolver->BlackHolePull(v3distance, a_pOther->GetMass());
		}
...
void Simplex::MyEntity::SetYPositionForGravityCheck(float a_fYPos)
{
	if (m_pSolver) m_pSolver->SetYPositionForGravityCheck(a_fYPos);
}

bool Simplex::MyEntity::IsDead()
{
	if (m_pSolver) m_pSolver->IsDead();
	else return false;
}
