...
void Simplex::MyEntityManager::Update(void)
{
	...
	for (uint i = 0; i < m_uEntityCount; i++)
	{
		if (m_mEntityArray[i]->IsDead()) RemoveEntity(i);
	}
}
void Simplex::MyEntityManager::AddEntity(String a_sFileName, String a_sUniqueID)
{
	...
	if (pTemp->IsInitialized())
	{
		...
		if (s_keyword1 != std::string::npos) {
			...
			pTemp->SetYPositionForGravityCheck(3.0f);
		}
		else if (a_sUniqueID == "Ground") {
			pTemp->SetEntityType(2);
		}
		else {
			// Blocks
			pTemp->SetEntityType(1);
			pTemp->SetYPositionForGravityCheck(1.0f);
		}
